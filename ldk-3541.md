...
Don't forget: 
- Make sure test assertions are as accurate as possible
- Add invalid onion blinding length check in separate commit
- Redo the u16 / failure_code stuff in onion_utils
  - use a macro or mapx to be msmarter
- Look at channel closed errors and decide whether to make those
  "upstream channel closed" / some of these are timeouts
- We should say when our peer has sent too little!
- Add a manual timeout option?
- Need to replace the temporary channel failure stuff

All failing tests for destination stuff:


    ln::blinded_payment_tests::blinded_path_retries
    ln::blinded_payment_tests::failed_backwards_to_intro_node
    ln::blinded_payment_tests::forward_checks_failure
    ln::blinded_payment_tests::forward_fail_in_process_pending_htlc_fwds
    ln::blinded_payment_tests::min_htlc
    ln::blinded_payment_tests::multi_hop_receiver_fail
    ln::blinded_payment_tests::three_hop_blinded_path_fail

Odd, value seems to zigzag
    ln::onion_route_tests::test_onion_failure

Index out of bounds issue
    ln::payment_tests::no_extra_retries_on_back_to_back_fail

Weird - is channel ready
    ln::payment_tests::retry_with_no_persist
    ln::payment_tests::test_payment_metadata_consistency
