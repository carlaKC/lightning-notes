...
How would we handle replay?
- 

Double encryption:

  Comprehensive Plan: Correct Trampoline Error Handling

  Background: The Two-Layer Encryption Requirement

  Per BOLTS PR #836, when an intermediate trampoline node (Carol) sends an error back to the sender (Alice), it must be
   double-encrypted:

  Payment: Alice →outer→ Bob →outer→ Carol ==trampoline==> [Dave →inner→ Eve]

  Error from Carol back to Alice:
    1. Build error packet, encrypt with Carol's trampoline_shared_secret  (inner layer)
    2. Encrypt again with Carol's outer_onion_shared_secret              (outer layer)

  Alice decrypts:
    1. Peel Bob's outer layer, Carol's outer layer  (outer onion keys)
    2. Peel Carol's trampoline layer, Eve's trampoline layer  (trampoline keys)
    → Recovers original error


  The existing get_encrypted_failure_packet already supports this via its secondary_shared_secret parameter -- it's
  just not being called correctly.

  ---
  Issue 1 (CRITICAL BUG): Missing double-wrapping in fail_htlc_backwards_internal

  Location: channelmanager.rs:9382-9433

  Bug: The TrampolineForward arm passes &None for trampoline_shared_secret at line 9429:

  // Line 9382: incoming_trampoline_shared_secret is consumed in `..`
  HTLCSource::TrampolineForward { previous_hop_data, outbound_payment, .. } => {
      ...
      get_htlc_forward_failure(
          ...
          &None,   // ← BUG: should be &Some(incoming_trampoline_shared_secret)
          &None,
          *htlc_id,
      )

  This means errors are only single-encrypted (outer onion) when they must be double-encrypted (trampoline + outer).

  Fix:
  1. Destructure incoming_trampoline_shared_secret from HTLCSource::TrampolineForward (instead of ..)
  2. Pass &Some(*incoming_trampoline_shared_secret) to get_htlc_forward_failure

  This triggers the two-layer path in get_encrypted_failure_packet (onion_utils.rs:2130-2142):
  - build_failure_packet(trampoline_secret, ...) → inner layer
  - process_failure_packet(outer_secret, ...) + crypt_failure_packet(outer_secret, ...) → outer layer

  ---
  Issue 2 (BUG): Blinded failure override for trampoline errors

  Location: channelmanager.rs:9417 and get_htlc_forward_failure at line 14122-14131

  Bug: When the trampoline node is also a blinded path introduction node, get_htlc_forward_failure checks
  blinded_failure from each incoming HTLC and overrides the trampoline error with invalid_onion_blinding:

  // get_htlc_forward_failure line 14123:
  Some(BlindedFailure::FromIntroductionNode) => {
      let blinded_onion_error =
          HTLCFailReason::reason(LocalHTLCFailureReason::InvalidOnionBlinding, vec![0; 32]);
      // ← WRONG for trampoline: should be temporary_trampoline_failure

  Per BOLTS test vector 2: when Carol is both a trampoline node AND blinded intro node, and Dave (in the blinded path)
  fails, Carol generates temporary_trampoline_failure -- NOT invalid_onion_blinding.

  Fix: In the TrampolineForward arm of fail_htlc_backwards_internal, pass &None for blinded_failure instead of the
  per-HTLC blinded_failure. The trampoline error has already been correctly determined (either forwarded recipient
  error or TemporaryTrampolineFailure).

  ---
  Issue 3 (CODE QUALITY): unwrap() on trampoline_shared_secret

  Locations:
  - channelmanager.rs:9053 -- MPP timeout: payment.htlcs[0].prev_hop.trampoline_shared_secret.unwrap()
  - channelmanager.rs:16070 -- on-chain timeout: same pattern

  Both have // TODO: no unwrap comments.

  Fix: Replace with:
  let incoming_trampoline_shared_secret = match payment.htlcs[0].prev_hop.trampoline_shared_secret {
      Some(secret) => secret,
      None => {
          debug_assert!(false, "trampoline forward missing trampoline_shared_secret");
          return true; // keep entry, skip failing
      }
  };

  ---
  Issue 4 (IMPROVEMENT): Distinguish UnknownNextTrampoline from TemporaryTrampolineFailure

  Location: handle_trampoline_htlc() at channelmanager.rs:~8285-8290

  Current: When send_payment_for_trampoline_forward fails, the error is always TemporaryTrampolineFailure:
  if let Err(_retryable_send_failure) = result {
      return Err((source, HTLCFailReason::reason(
          LocalHTLCFailureReason::TemporaryTrampolineFailure, vec![]
      )));
  }

  Spec says:
  - Cannot reach next trampoline at all → unknown_next_trampoline (PERM|27) -- tells sender to find a different
  trampoline
  - Routes found but temporarily failed → temporary_trampoline_failure (NODE|25) -- sender can retry

  Fix: Inspect the RetryableSendFailure variant:
  if let Err(retryable_send_failure) = result {
      let failure_reason = match retryable_send_failure {
          RetryableSendFailure::RouteNotFound =>
              LocalHTLCFailureReason::UnknownNextTrampoline,
          _ => LocalHTLCFailureReason::TemporaryTrampolineFailure,
      };
      return Err((source, HTLCFailReason::reason(failure_reason, vec![])));
  }

  ---
  Issue 5 (DEFENSIVE): process_onion_failure panics if outbound_payment is None

  Location: onion_utils.rs:1033-1035

  HTLCSource::TrampolineForward { outbound_payment, .. } => {
      let dispatch = outbound_payment.as_ref()
          .expect("processing trampoline onion failure...");

  The callers (trampoline_htlc_failed) guard against this, but the expect is fragile.

  Fix: Return a DecodedOnionFailure with payment_failed_permanently: true instead of panicking.

  ---
  Issue 6: Recipient error forwarding is correct but narrowly scoped

  Location: channelmanager.rs:9393-9402

  is_recipient_failure() (onion_utils.rs:1797) checks: IncorrectPaymentDetails, FinalIncorrectCLTVExpiry,
  FinalIncorrectHTLCAmount, MPPTimeout.

  Assessment: This is currently correct since our outbound payments use regular routing (not trampoline), so we'll
  never receive trampoline-specific errors from downstream. All downstream errors are regular onion errors.
  Non-recipient errors are correctly converted to TemporaryTrampolineFailure.

  Future work: If multi-hop trampoline is added, trampoline errors from downstream should also be forwarded (not just
  recipient errors).

  ---
  Issue 7: Test coverage needed

  The unit test test_trampoline_onion_error_cryptography (onion_utils.rs:3761) validates the cryptographic layer.
  Integration tests are needed for:

  1. Double-wrapping roundtrip: sender → trampoline generates error → sender decrypts both layers and recovers error
  code
  2. Recipient failure forwarding: recipient sends IncorrectPaymentDetails → trampoline forwards it → sender sees
  original error
  3. Non-recipient conversion: intermediate hop fails with FeeInsufficient → trampoline converts to
  TemporaryTrampolineFailure
  4. Blinded + trampoline interaction: trampoline is blinded intro node → failure from blinded path → sender sees
  TemporaryTrampolineFailure (NOT InvalidOnionBlinding)
  5. MPP timeout with wrapping: trampoline MPP times out → error is double-wrapped correctly
  6. Fee/expiry insufficient: trampoline rejects for fee budget → sender sees TrampolineFeeOrExpiryInsufficient with
  correct 10-byte data
  ...
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 1    │ Fix double-wrapping (Issue 1)               │ ~5 lines     │ Critical correctness               │
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 2    │ Fix blinded failure override (Issue 2)      │ ~3 lines     │ Correctness for blinded+trampoline │
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 3    │ Remove unwraps (Issue 3)                    │ ~10 lines    │ Prevents panics                    │
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 4    │ Distinguish UnknownNextTrampoline (Issue 4) │ ~8 lines     │ Better sender signal               │
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 5    │ Defensive process_onion_failure (Issue 5)   │ ~10 lines    │ Code quality                       │
  ├──────┼─────────────────────────────────────────────┼──────────────┼────────────────────────────────────┤
  │ 6    │ Add integration tests (Issue 7)             │ Bulk of work │ Validates all above                │
  └──────┴─────────────────────────────────────────────┴──────────────┴────────────────────────────────────┘

  Issues 1+2 together are ~8 lines of production code change but are critical for spec compliance. The test work (Issue
   7) is the largest effort.

✻ Cogitated for 53s

⏺ Agent "Fetch BOLTS PR 836 spec" completed

⏺ All three research agents have completed. The plan above is comprehensive and ready. Let me know if you have
  questions about any of the issues, want to adjust priorities, or are ready to start implementing. The highest-impact
  changes are Issues 1 and 2 (the double-wrapping fix and blinded failure override) -- together they're ~8 lines of
  production code but are critical for spec compliance.

⏺ Agent "Analyze current PR commits" completed

⏺ All three research agents are now complete. The plan above covers everything -- let me know if you want to dive into
  implementing any of these issues, or if you have questions about the approach.
