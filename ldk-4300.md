...
### f drop stale lockorder comments

Clean up comments no longer needed.

### f clearer control flow

Rename the macro to include "continue"

### f drop unused diff

remove mut where not used

### Support generic HTLC interception

Reviewed fixups below. Here reviewing them squashed (without the flag
enum added).

`do_test_htlc_interception_flags`:
- Takes a `flags_bitmask` which represents the htlcs we want to intercept
- Takes a flag_big, which is the type of htlc we want to test against
- Takes `ForwardingMods` that I assume change the HTLC (?)

- Create 3 channels: `A -- B -- C`, with `B` setting the intereception
  config in its channelmanager
- Now, we open up the right type of channel from `B-C` (private if
  testing a private HTLC, otherwise public). If testing offline,
  disconnect the node. Also set a fake SCID if necessary.
- Set `interception_bit_match` = whether we're expecting an intercept
- Apply relevant modifications to route to hit failure mods if required
- Lock the HTLC in on the first hop
- If intercepting and expecting a HLTC to be forwarded:
  - Get the intercepted HTLC 
  - If we had an offline interception:
    - Reconnect the nodes
  - Resume the intercept, claim the payment
- If we are not intercepting, and expected the flag to succeed
  - Assert success and claimable
- Otherwise:
  - Assert failure with the error we expect

Q: Why do we want to test `CLTVBelowMin` for non-intercept as well?
- It should fail regardless of whether we're intercepting

Q: What if intercepeted = true and mods = cltv?
- We'll fall into the failure branch.

### f sp

Trivial spelling fixup.

### f refactor forward-checking api

- Breaks `forward_needs_intercept` up into `to_known_chan` and
  `to_unknown_chan` variants.
- Passes a `will_intercept` bool into `can_forward_htlc_to_outgoing_channel`
  so that interception logic is separate from validation.
...
Feels like we're using an unnatural representation that we have to
fiddle with it so much in tests.
