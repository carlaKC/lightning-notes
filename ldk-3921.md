...
- `excess_fees_on_counterparty_tx_dust_exposure_msat` returns the
  additional fees on the counterparty's commitment that we consider
  to be "excessive" (ie, above a normal fee rate that we think is sane).
  This is calculated using the marginal excess (ie, the difference):
  - Return this fee for adding _one more_ htlc to their commit
  - Return this fee for the current set of commitment htlcs

- `get_next_commitment_stats`
  - Calculates the `excess_feerate_opt` as the current feerate less the
    dust limiting value (+ asserts correct for V3)
  - `value_to_counterparty`: channel value less our balance, this can
    always be expected because only we manage this balance
  - `outbound_htlcs_value_msat` and `inbound_htlcs_value_msat`
    subtracted from local and remote balances respectively

Q: are these outbound and inbound *always* considered from our
perspective?
  - Yes, we always subtract outbound from our balance and inbound
    from their balance

Q: `value_to_holder_after_htlcs_msat` should never overflow because if
  it does then we've got a serious bug somewhere. Do we check this
  for the callers?
  [ ]

  Q: Where is this being used?
  - `validate_udpate_add_htlc`
    - Prev: pending_value_to_self_msat = 
      value_to_self_msat + pending_inbound_htlcs_value_msat 
      - removed_outbound_total_msat`
      - Same check  
    - Now: `holder_balance.unwrap_or(0) < reserve + commit_fee_msat`
  - `can_send_update_fee`:
    - Prev: `local_balance_before_fee_msat - outbound_holding_cell`
      then the same check
    - Now: `holder_balance.unwrap_or(0) < commit_fee_msat + reserve`
  Q: What did we previously do with small holder balances?
    - In debug mode we'd panic if we underflow, in release we'd just
      underflow if our value to self wasn't consistent

Q: How are we calculating `holder_balance_msat`?

Previously:
- Create a candidate htlc with the amount being added
- Get commitment fee including the htlc
- `local_balance_before_fee_msat`: funding plus inbound less reserved

Now:
- Take `value_to_holder_msat` input
- Subtract all outbound htlcs on commitment (checked)
- Subtract anchors (checked)

### Adjust dust exposure due to excess fees for clarity

Looked at this in previous rounds, unchanged.

### Add ChannelContext::get_next_{local, remote}_commitment_stats

Reviewed fixup commit to prevent overflow, looks good.

### Improve prediction of commitment stats in validate_update_add_htlc

Review fixups:
- One trivial variable use fixup
- One code move

### Improve prediction of commitment stats in can_accept_incoming_htlc

Review fixups:
- Just a change to the variable name and comment position

### Improve prediction of commitment stats in `can_send_send_update_fee`

Review fixup:
- Restore check on local commitment, rather than duplicate on the
  remote.

### Remainder

Last two commits untouched.
