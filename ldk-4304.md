...
Range diff:
New PR: c1e0cf45f..8ce1604ae
Pre rebase: 7ebf13bf6..f15a1c5a3

Squashed and rebased. 

## Address Review

[x] Make sure that no writes fail in the commits that follow
- Fixed up one place where we hit this error, checked that we don't
  have any other instances.

[x] Should be move `prev_channel_ids` from `HTLCHandlingFailed` into
  the `HTLCHandlingFailureType`
- I don't think so, it would fix up the helper but at the expense of
  crowding the event api unnecessarily.

[x] Can we use `FnOnce` in `claim_funds_from_htlc_forward_hop`
-  Next commit needs to call it multiple times? It's fine!

[x] Remove `prune_forwarded_htlcs`

[x] Can we validate that each `HTLCSource` with multiple `prev_htlcs`
    has the same htlcs?
- Comment is bad, just need to fix that.

[~] Should we have multiple channels in
  `EmitEventOptionAndFreeOtherChannel`?
- Not sure I understand this comment. Don't think that it makes sense.
  - Seems like we're on the same page

## Address Review

[ ] Are we going to switch to using payment claiming blockers rather
    than forwarding ones?

Q: How is `ClaimedMPPPayment` used?
- `claim_payment_internal`
  - Push a `MonitorUpdateCompletionAction::PaymentClaimed`
  - Push `RAAMonitorUpdateBlockingAction::ClaimedMPPPayment`
- `handle_monitor_update_completion_actions`
- `handle_in_flight_updates`

I don't think that we need these because we have an outbound payment,
but let's see what Matt says.

## Address Review

Q: Can we move processing of `RAAMonitorUpdateBlockingAction` into
  `claim_funds_internal`?
- Currently added in `internal_update_fulfill_htlc` (which then calls
  `claim_funds_internal`.
- `claim_funds_internal` is also called in `process_pending_monitor_events`
  when we have `MonitorEvent::HTLCEvent`
  - Called by `get_and_clear_pending_msg_events` (this only happens when
    the channel is already closed apparently, so should be fine).
